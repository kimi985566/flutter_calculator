...
class CalculatorState extends State<StatefulWidget> {
  ...
  onResultButtonPressed(display) {
    if (results.length > 0) {
      ...
      if (display == '=') {
        result.result = result.operator.calculate(
  ...
  onOperatorButtonPressed(Operator operator) {
    if (results.length > 0) {
      var result = results[results.length - 1];
      if (result.result != null) {
        var newRes = Result();
        newRes.firstNum = currentDisplay;
        newRes.operator = operator;
        results.add(newRes);
  ...
  onNumberButtonPressed(Number number) {
    ...
    if (result.firstNum == null || result.operator == null) {
  ...
  pickCurrentDisplay() {
    this.setState(() {
      ...
      if (results.length > 0) {
        ...
        } else if (result.secondNum != null && result.operator != null) {
